...
struct binder_node {
	...
	//所有引用binder_node的binder_ref
	struct hlist_head refs;
	int internal_strong_refs;
	int local_weak_refs;
	int local_strong_refs;
	//ptr指向该Service组件内部的引用计数
...
static size_t binder_buffer_size(struct binder_proc *proc,
				 struct binder_buffer *buffer)
{
	//binder_buffer描述的内核缓冲区由两块数据组成，一个是元数据块，一个是有效数据块
	//如果其是列表中的最后一个元素，那么有效数据就从data开始，直接拿结尾减去data就是大小
	if (list_is_last(&buffer->entry, &proc->buffers))
		return (u8 *)proc->buffer +
			proc->buffer_size - (u8 *)buffer->data;
	//否则就等于下一个内核缓冲区的起始地址减去成员变量data的地址
						  ...
						  uintptr_t user_ptr)
{
	...
	//在已分配的红黑树里面查找，找得到就说明是有效的缓冲区地址，否则就是NULL
	while (n) {
...
//为指定的虚拟地址空间分配或者释放物理页面
//proc  指向要操作的进程
//allocate 表示要释放/分配的物理页面
//start - end表示要操作的内核空间的开始和结束地址
//vma表示要映射的用户地址空间
					      ...
					      int is_async)
{
	...
	//将data_size和offsets_size对齐到指针边界，然后相加得到总缓冲区大小
	...
	//如果是异步，要进一步检查分配的内核缓冲区大小是否大于目标剩余的可用于异步事务的内核缓冲区大小
	if (is_async &&
	...
	//开始对buffer大的那一部分进行裁剪，如果裁剪过后小于4个字节就不裁剪了
	end_page_addr =
	...
	if (buffer_size != size) {
		//给buffer裁剪后剩的部分重新构建binder_buffer
		struct binder_buffer *new_buffer;
...
//创建binder_node节点
//cookie 指向Binder本地对象的弱引用计数对象的地址
//ptr 指向Binder本地对象的地址值
...
//strong 表示要增加的强引用还是弱引用技术
//internal  增加内部还是外部引用计数
				   ...
				   struct binder_thread *thread)
{
	...
	if (security_binder_transfer_binder(proc->tsk, target_proc->tsk))
		return -EPERM;
	//在目标进程target_proc创建一个Binder引用对象引用该Node
	ref = binder_get_ref_for_node(target_proc, node);
	if (!ref)
		return -EINVAL;
	//将flat_binder_object的类型改为BIDNER_TYPE_HANDLE、

	if (fp->hdr.type == BINDER_TYPE_BINDER)
		fp->hdr.type = BINDER_TYPE_HANDLE;
	else
		fp->hdr.type = BINDER_TYPE_WEAK_HANDLE;
	//设置其句柄值，binder实体对象已经变成binder引用对象了
	fp->binder = 0;
	fp->handle = ref->desc;
	fp->cookie = 0;
	//防止在传递的过程中被销毁，增加其引用计数
...
//负责处理BC_REPLY和BC_TRANSACTION
//reply用来区分是处理的BCTRANSACTION还是BC_REPLY
//binder_proc和binder_thread代表发出命令的主进程和主线程
			       ...
			       binder_size_t extra_buffers_size)
{
	...
	if (reply) {
		...
		//寻找到空闲的二类线程，只有同步的请求才会考虑这个方案，因为异步的进程间请求不会涉及一个线程等待另一个线程
	...
	//找到一个最优的目标线程target_thread来接收BR_TRANSACTION
	if (target_thread) {
		//分别指向目标进程的target_thread的todo队列，和wait等待队列
	...
	if (reply)
		...
		binder_debug(BINDER_DEBUG_TRANSACTION,
			     ...
			     (u64)tr->data_size, (u64)tr->offsets_size,
			     (u64)extra_buffers_size);
	//如果描述的是一个同步的进程间通信请求，将t的from指向源线程thread
	if (!reply && !(tr->flags & TF_ONE_WAY))
		t->from = thread;
	else
		t->from = NULL;
	//初始化binder_transaction结构体t
	...
	//偏移数组的开始位置
	offp = off_start;
	//把binder_transcation_data的数据缓冲区拷贝到t的缓冲区中
	...
	//把binder_transcation_data的偏移缓冲区拷贝到t的缓冲区中
	...
	}//处理BC_TRANSACTION ,且是同步请求 
	else if (!(t->flags & TF_ONE_WAY)) {
		BUG_ON(t->buffer->async_transaction != 0);
		//同步请求 设置need_reply为1
		t->need_reply = 1;
		t->from_parent = thread->transaction_stack;
		//将事务t压入事务堆栈transaction_stack中
		thread->transaction_stack = t;
	} else {
		BUG_ON(target_node == NULL);
		BUG_ON(t->buffer->async_transaction != 1);
		//是否正在处理异步事务，如果有的话 就要放倒async_todo中处理
	...
	//把BINDER_WORK_TRANSACTION放到目标进程的工作队列中
	t->work.type = BINDER_WORK_TRANSACTION;
	list_add_tail(&t->work.entry, target_list);
	//把一个BINDER_WORK_TRANSACTION_COMPLETE放到源线程的工作队列中
			...
			binder_size_t *consumed)
{
	...
	while (ptr < end && thread->return_error == BR_OK) {
		...
		//注册服务时的BC_TRANSACTIOn
		case BC_TRANSACTION:
		case BC_REPLY: {
			struct binder_transaction_data tr;
			//把数据拷贝到binder_transaction_data中
		...
		case BC_ENTER_LOOPER:
			binder_debug(BINDER_DEBUG_THREADS,
				     "%d:%d BC_ENTER_LOOPER\n",
				     proc->pid, thread->pid);
			//BINDER_LOOPER_STATE_REGISTERED和BINDER_LOOPER_STATE_ENTERED是互斥的
				...
				struct binder_thread *thread)
{
	...
	//BINDER_LOOPER_STATE_NEED_RETURN也是需要立即返回用户空间，所以也返回1表明有工作项需要处理，防止进入睡眠状态
		(thread->looper & BINDER_LOOPER_STATE_NEED_RETURN);
}

static int binder_has_thread_work(struct binder_thread *thread)
{	
	//上次处理工作项遇到了错误，返回值不是BR——OK，也要返回1 ，防止阻塞
	return !list_empty(&thread->todo) || thread->return_error != BR_OK ||
		(thread->looper & BINDER_LOOPER_STATE_NEED_RETURN);
}


//目标线程会不断地调用Binder驱动程序中的binder_thread_read来检查todo中的新的工作项
...
retry:
	//transaction_stack不为NULL，表示等待其他线程完成另外的事务
	//todo 不为空，说明线程还有未处理的工作项
	//线程必须先处理自己的这两个队列中的事务
	//可以处理proc中的todo的标志
	wait_for_proc_work = thread->transaction_stack == NULL &&
				list_empty(&thread->todo);
	//上次处理工作项时返回值不是BR_OK，说明出现了错误，也需要立即返回
	...
	//如果wait_for_proc_work == 1 说明当前线程所属的进程多了一个空闲的Binder线程
	...
	if (wait_for_proc_work) {
		...
		//non_block表示当前线程是否是以非阻塞模式打开的设备文件/dev/binder
		//如果是非阻塞模式，当前县城就不可以在Binder驱动模式种睡眠，因为非阻塞么，没有需要处理的工作项就立即返回
		if (non_block) {
			//判断进程是否有未处理的工作项
	...
	while (1) {
		...
		//检查目标进程或者目标线程中的todo队列，将里面待处理的工作项保存到w中
		...
		if (end - ptr < sizeof(tr) + 4)
			break;
		//注册服务时，会处理一个BINDER_WORK_TRANSACTION_COMPLETE的工作项
		switch (w->type) {
		...
		//协议BR_INCREFS、BR_ACQUIRE BR_DECREFS 和BR_RELEASE
	...
	*consumed = ptr - buffer;
	//是否需要请求当前proc创建一个新的线程
	//需要满足条件 proc的ready_threads空闲线程数等于0
	//requested_threads等于0 没有正在请求增加新线程
	//requested_threads_started小于预设的最大数量
	//当前线程得是一个注册了的线程，要不然没人处理其写入的BR_SPAWN_LOOPER命令
...
//获取当前线程的binder_thread结构体，
static struct binder_thread *binder_get_thread(struct binder_proc *proc)
{
	struct binder_thread *thread = NULL;
	struct rb_node *parent = NULL;
	struct rb_node **p = &proc->threads.rb_node;
	//首先会检查线程对应的binder_thread是否存在
	while (*p) {
		parent = *p;
	...
	if (*p == NULL) {
		...
		//设置状态，表示需要马上返回用户空间，而不可以去处理进程间通信请求
		thread->looper |= BINDER_LOOPER_STATE_NEED_RETURN;
		thread->return_error = BR_OK;
		thread->return_error2 = BR_OK;
	}
...
static long binder_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	binder_lock(__func__);
	//为当前线程创建一个binder_thread 结构体，SM主线程第一次进来的时候就该为其创建一个binder_thread结构体
	thread = binder_get_thread(proc);
	if (thread == NULL) {
		ret = -ENOMEM;
		goto err;
	...
	case BINDER_WRITE_READ: {
		...
		//注册线程时BC_ENTER_LOOPER协议，这里等于0 ，不会进去处理
		if (bwr.read_size > 0) {
	...
	case BINDER_SET_CONTEXT_MGR:
		//binder_context_mgr_node里保存的就是sm，不为空说明已经注册过了
	...
	ret = 0;
err:
	//清楚NEED_RETURN状态，下次线程进入Binder驱动就可以处理请求了
	if (thread)
		thread->looper &= ~BINDER_LOOPER_STATE_NEED_RETURN;
	binder_unlock(__func__);
...
{
	int ret;
	//vm_area_struct描述的是用户空间  vm_struct描述的是内核空间
	struct vm_struct *area;
	//从filp里面取出binder_proc
	struct binder_proc *proc = filp->private_data;
	...
	if (proc->tsk != current->group_leader)
		return -EINVAL;
	//vma_start和vma_end指定了要映射的用户地址范围,如果超过4M就截断
	...
	//分配成功用binder_buffer来描述它 data就指向buffer
	buffer->data = proc->buffer;
	list_add(&buffer->entry, &proc->buffers);
	buffer->free = 1;
	//加入到porc中的空闲内核缓冲区红黑树 free_buffers里面
	binder_insert_free_buffer(proc, buffer);
...
static int binder_open(struct inode *nodp, struct file *filp)
{
	...
	//把binder_proc保存到文件描述符的private_data里面
	filp->private_data = proc;

	binder_unlock(__func__);
	//在binder的debugfs里面创建一个以进程ID为名称的只读文件
	//可以读取到Binder线程池、Binder实体对象以及内核缓冲区信息
...
static int __init binder_init(void)
{
	...
	if (!binder_deferred_workqueue)
		return -ENOMEM;
	//创建一个/procb/binder/proc目录
	binder_debugfs_dir_entry_root = debugfs_create_dir("binder", NULL);
	if (binder_debugfs_dir_entry_root)
		binder_debugfs_dir_entry_proc = debugfs_create_dir("proc",
						 binder_debugfs_dir_entry_root);

	//又在/proc/binder下面创建了5个文件 state  stats  transactions  transactions_log 和failed_transaction_log
	//通过这5个文件可以读取binder的运行状态
	if (binder_debugfs_dir_entry_root) {
	...
	printk(KERN_DEBUG,"BINDER: device_name: %s",device_names);
